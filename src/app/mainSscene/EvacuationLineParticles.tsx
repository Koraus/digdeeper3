...
export function EvacuationLineParticles({
    width, ...props
    ...
    const { mesh, setTimeSec } = useMemo(() => {
        ...
        ig.instanceCount = Math.ceil(width * 20);
...
void particleMain() {
    ...
    mat4 emitterTransform = translate(vec3(0.0, 0.5, 0.0));
    ...
    vec3 initialPosition = mix(
        vec3(-0.5, -0.5, ${-float(width / 2)}), 
        vec3(-0.2, 0.5, ${float(width / 2)}), 
        random3());
